...
- **Status:** **IMPLEMENTED - ACs verified, and now WIRED (2026-09-21)** · `test_us013_breaker.py` **70/70** · `test_brd15_rollback.py` demonstrates the revert · DoD 2/8 · **A3 done (2026-09-19): the breaker's state is operator-readable on `GET /api/perf/policy` under `retrieval`** (additive; pure in-memory snapshot, never a call; no credential value) · **T-11 done (2026-09-21): the per-turn `DAT-07` record carries `retrieval_rung`, `retrieval_breaker` and `retrieval_dead_dependency_ms`** · **the breaker now gates the serving path** — until 2026-09-21 it gated nothing, and this status line was wrong to say "ACs verified" without qualification (see the wiring note below)
...
- **Components touched:**
  ...
  - `MOD-06` — **claim NOT implemented as written (found 2026-09-19).** This says the record carries the rung that served, the breaker state, and the wall time charged to a dead dependency. It does not: `retrieval_rung`, `retrieval_breaker` and `retrieval_dead_dependency_ms` appear nowhere in `app/` — zero matches across `perf_trace.py`, `voice_handler.py` and `rag.py`. What exists is the breaker's *aggregate* counters via `mcp_rag_status()` (`breaker_state`, `breaker_opens`, `breaker_probes`), which are process-lifetime totals and cannot attribute cost to a turn. So the outage cost is **inferred, not measured**, which is the opposite of what this line claims and of what `BRD-01` requires. The DoD box below says as much in its own words. Closing it meant adding the three fields to the trace — a change to the record schema.

**CLOSED 2026-09-21, and closing it found something worse.** Adding the fields is what made the wiring defect visible, because the fields *measure* it. `_retrieve_context` — the only place the voice path chooses a rung — never called `rag._use_mcp()`, the function written to make that choice. `_use_mcp` had no caller in `app/` at all: `git log -S` across the tree shows the name in exactly one commit, `a049bc5`, the one that defined it. So the breaker recorded every outage honestly and gated nothing. Each turn during an outage called the dead service at the full 6.0 s serving read timeout; `claim_probe()` was never reached, so `_probe_in_flight()` was never true, so `_post` never selected the 1.5 s probe budget and `breaker_probes` could only ever read 0. **TAC-4's "bounded per window, not per request" was untrue as wired** — cost grew with every turn served — and `test_us013_breaker.py` passed 51/51 throughout, because it calls `_use_mcp` and `claim_probe` directly: it tested the mechanism, not its reachability.

The fix routes all three primary call sites through one decision, `rag_mcp.admit_primary()`: `_retrieve_context`, `_threshold_distance` (a second primary call inside the same turn, dormant only because `RAG_SIMILARITY_THRESHOLD` is 0.0) and `MCPRetriever` (the `app.py`/`admissions_bot.py` chat surfaces). Gating the last two matters beyond their own budget: `_record_failure` restamps `failed_at` on every failure, so an ungated caller keeps pushing the cooldown out and the half-open probe never becomes eligible. Six new checks drive `_retrieve_context` itself and fail if the wiring is removed again — verified by reverting `app/rag.py` alone and watching them report `the primary was called 1 time(s) anyway`, `2 probes for one window` and `probe budgets seen: [False, False]`.
- **Interaction summary:**
...
| T-11 | integration | ~~The record shows the rung, the breaker state and the wall time charged to the dead dependency (TAC-4)~~ | **PASS (2026-09-21).** `app/rag.py::_note_retrieval` writes all three fields on every return path of `_retrieve_context`, so the record now attributes outage cost to the turn a caller waited on instead of leaving it to be inferred from `mcp_rag_status()`'s process-lifetime totals. Nine checks in `test_us013_breaker.py` cover the healthy turn (rung `mcp`, cost `0.0`), the refused turn (breaker `open`, cost `0.0` — this is TAC-4's "bounded per window") and the admitted probe (breaker `half_open`, cost **measured**). Adding the fields is what exposed the wiring defect below: they would have recorded cost growing with every turn |
