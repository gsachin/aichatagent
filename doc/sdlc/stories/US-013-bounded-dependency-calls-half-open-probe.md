...
- **Status:** **IMPLEMENTED - ACs verified** · `test_us013_breaker.py` 51/51 · `test_brd15_rollback.py` demonstrates the revert · DoD 2/8 · **A3 done (2026-09-19): the breaker's state is now operator-readable on `GET /api/perf/policy` under `retrieval`** (additive; pure in-memory snapshot, never a call; no credential value) — the per-turn `DAT-07` trace fields (T-11) remain open
