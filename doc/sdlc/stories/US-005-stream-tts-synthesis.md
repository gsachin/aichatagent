...
- **Status:** **PARTIAL — BUILT BEHIND `TTS_STREAM` (2026-09-20); adoption BLOCKED - `DG-03` (changes what the caller hears)** · `test_us005_stream_tts.py` 27/27 — identical under `TTS_CACHE_SCOPE=shared` and `per_call`, because the suite forces the scope it tests rather than inheriting ambient `.env`. The two AC-1/AC-2 cache checks had been seeding the shared dict while ambient `per_call` sent the lookup elsewhere, so they failed for a reason unrelated to their claim, and this line quoted the shared-scope run as the ambient one. The count is deliberately NOT bolded: `test_doc_truth.py` matches `` `<suite>.py` <n>/<n> `` and an intervening `**` makes the claim invisible to the gate rather than wrong · batch path retained for BRD-15 (the flag is the revert) · Live N=2 warm, stream on: first-audio **p50 2,250 ms** (batch 2,453), **p90 3,360** (batch 3,891), over-cap **13%** (batch 27%); `llm_done → tts_first_chunk` **p50 438 ms** (target ≤400) · Known limit, recorded: kokoro's own stream batches a whole short answer into ONE chunk, so streaming synthesises PER SENTENCE (`_split_sentences`); sentence 2+ synthesise after sentence 1's frames are written, not in true parallel — the full overlap arrives with US-004's clause cutter, whose interface this stream already is
