...
- **Status:** **PARTIAL - latency leg measured; quality leg SCREEN-GRADE only (not adoption-grade); the funnel and its Pareto front are NOT STARTED**

  **"PARTIAL" understated this until 2026-09-19.** The story's central deliverable is a staged funnel with a Pareto front and documented eliminations. None of it exists: `doc/perf/tools/model_funnel.py`, `pareto_front()`, `class Candidate`, `class Stage`, `class Elimination` and `doc/perf/model_candidates.json` are all absent, and the word "pareto" appears in the repository only inside a docstring title. What does exist is the latency benchmark and a screen-grade quality leg, which is why the status said PARTIAL — but a reader taking PARTIAL as "most of it is there" would be wrong, so the status now names the missing third. The quality leg's own state is also narrower than "blocked": a screen was built and run (`doc/perf/tools/us015_quality_screen.py`) and it separated the two candidates on mechanical defects — what `DG-03` blocks is the *adoption-grade* verdict, not any measurement at all.
