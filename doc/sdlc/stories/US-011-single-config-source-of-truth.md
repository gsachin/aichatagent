...
- **Status:** **IMPLEMENTED - TAC-5 wired 2026-09-19; TAC-1 MIGRATION COMPLETE 2026-09-21 (Phase 1.2, zero unexplained reads; bucket D closed); AC-4 still partial; BRD-15 demonstrated** · `test_us011_config_truth.py` **55/55** · `test_us011_import_order.py` 13/13 · **DoD 6/8** · **box 2 is driven** (14 pass / 1 partial / 2 not run — the partial is T-8, excluded deliberately) · **only box 3 remains**: the TAC-8 no-regression load pair, ~2–4 h, and it needs a quiet box · **TAC-6 implemented 2026-09-21** — provenance reported a constant until then, and the check asserting otherwise was enforcing the defect · **T-15 closed 2026-09-21** — `ConfigurationError`, specified in this story's LLD since it was written and never implemented, now names the key from every import-time parse
...
| T-7 | unit | `FASTAPI_WORKERS` reports `in_effect: false` with the `REC-02` reason, or is wired so the runtime honours it — asserted as one of the two, not neither | **`in_effect: false` — satisfied**, but only after the launcher was fixed. The dichotomy was not the error it first looked like: the second half was reachable all along and `start.sh` was what made the answer CONDITIONAL. The flag is now removed and the boot report says NOT IN EFFECT; see Phase 1.2 below |
...
the checks that actually exist: **fourteen pass, one is partial, two are not
run** (T-13/T-17, the TAC-8 load pair). The remaining partial is named with its
reason rather than averaged away — a scenario marked PARTIAL is one not fully
exercised even where the underlying behaviour is right, which is the distinction
this story exists to keep.

**Mapping T-1 and T-10 found that TAC-6 was not implemented, and that a passing
check was enforcing the defect.** TAC-6 names three origins — the authoritative
file, a documented code default, and the detection artifact consulted and
overridden. `effective_configuration()` reported **one**. It enumerated `.env`'s
keys and stamped every row `"authoritative (.env)"`, so `source` was a
*constant*, and two consequences followed:

- the **58 `Settings` fields with no `.env` entry** — every documented default —
  did not appear in the report at all, so "why is it this value" had no answer
  for them, and that question is what TAC-6 exists to answer; and
- a key the artifact also carried was indistinguishable from one it did not, so
  `overridden_detection_artifact` was unreachable and T-1's scenario could not
  be satisfied by any amount of testing.

The check named *"AC-1 every key reports a provenance"* asserted
`all(s.startswith("authoritative"))` — which was true only because the value was
a constant. **It would have failed the moment provenance was implemented
correctly**, so the suite was holding the defect in place: 58 keys correctly
reporting "code default" is what broke it, not a regression. The row set is now
the union of `.env`'s keys and `Settings`' fields (64 + 58 = **122 rows**), and
`source` is computed from which of the two supplied the value. The three real
divergences the report had been hiding are now visible — `OLLAMA_MODEL`
(`applied=qwen2.5:14b` vs `llama3.2:3b`), `WHISPER_NUM_THREADS` (`6` vs `4`) and
`LLM_PROVIDER` — which are precisely the `REC-05`/`DG-05` disagreements this
module was built to surface.

| # | Status | Evidence, or what is missing |
|---|---|---|
| T-1 | **PASS — closed 2026-09-21** | It was PARTIAL for two reasons, both now fixed. `overridden_detection_artifact` is produced: a key the artifact carries *and* `.env` overrides now records `overridden_detection_artifact (applied=…)`, so the divergence is readable and not merely flagged. And the check that reads as its evidence — *"the authoritative file is .env, not the detection artifact"* (`doc/perf/tools/test_brd15_rollback.py:323`) — asserted only that every `source` was one of three known strings, so it **would have passed with every key sourced from the artifact**. It now tests the keys where the two actually disagree, which is what "`.env` wins" means. |
...
| T-8 | **PARTIAL — deliberately, and the reason is stated** | *"changing it changes the effective value, with no code change"* demonstrates it **in-process**. The "after a restart" half is not driven, and driving it would mean writing to `.env` — the untracked runtime authority, with no VCS safety net, in a test that could corrupt it on a crash. The property a restart would re-run is the resolution path itself, which the in-process check exercises. Left partial rather than faked. |
...
| T-10 | **PASS — closed 2026-09-21** | It was not merely untested but **unreachable**: the row set was `.env`'s keys, so removing the file produced an *empty report*, and "documented default" was not an origin the code could express. Now covered by four checks, which rebuild `Settings()` with the keys stripped from `os.environ` as well as the file — patching the file reader alone would leave `settings` serving configured values while the report claimed "code default", a green test asserting the wrong thing. Non-vacuity shown against the old implementation, which yields **0 rows** here. |
| T-11 | **PASS — closed 2026-09-21** | Driven as the rename it describes: a renamed key's NEW name is read while the OLD name, still in the file, is reported inert — so a rename cannot leave a silently-ignored setting behind. Both halves asserted, and the live name is asserted *not* to be reported. |
| T-12 | **PASS — closed 2026-09-21** | The five instances are now asserted individually: the four named keys each resolve to a reader and state a known origin, and `REC-01`'s dead Pipecat `VADParams` block is checked to be **gone from `app/pipeline.py`, not merely marked** — the distinction that row was really about, since a dead block that reads as live endpointing configuration was the most dangerous inert value in the repository. |
...
| T-15 | **PASS — closed 2026-09-21** | It was PARTIAL because no single path had both halves: a key parsed at import stopped the stack but named only the VALUE inside a traceback; a key that survived import was named properly by `check_config_keys`, but that made the stack not-READY rather than stopping it. **`app/config.py` now owns both.** `ConfigurationError` — specified in this story's LLD since it was written ("raised at **start**, never at first use, and naming the key") and never implemented — now exists, and `_env_int` / `_env_float` raise it from the 46 inline `int(_env(...))` / `float(_env(...))` sites they replaced. The message is `VAD_SILENCE_MS='notanumber' is not an integer (default 600). Fix it in .env, or remove the key to use the default.` Four checks cover it, including a control asserting a **valid** value still boots — so the test cannot pass because the child died for an unrelated reason. Non-vacuity shown by reverting `app/config.py` alone: the stop passes, the naming fails with `ValueError: invalid literal for int() with base 10: 'notanumber'` — T-15's finding, stated by the test itself. **The 46-site refactor was proven behaviour-preserving** by comparing all 110 resolved `Settings` fields against a pre-change snapshot; that comparison caught a real regression the diff would not have shown (6 fields silently changed type `float → int`, because defaults written `"2"` rather than `"2.0"` became uncoerced int literals). |
...
- [ ] Tests from the LLD test scenarios pass (T-1 … T-17) — **mapped and driven 2026-09-21: 14 pass, 1 partial, 2 not run** (evidence table above). T-14, T-1, T-10, T-11, T-12 and **T-15** are closed; closing T-1/T-10 exposed TAC-6 being unimplemented, and closing T-15 implemented the LLD's `ConfigurationError`. The box stays open on **T-8** only (the restart half would mean writing the untracked `.env` from a test — left partial deliberately), plus the TAC-8 load pair
...
|---|---|---|---|---|
| 2 | LLD test mapping (T-1…T-17) | **Mapped and driven 2026-09-21**: T-14, T-1, T-10, T-11, T-12 and **T-15** closed. Closing T-1/T-10 exposed **TAC-6 being unimplemented** (`source` was a constant; the report is now 122 rows with computed provenance, surfacing three real `.env`-vs-artifact divergences). Closing T-15 implemented the LLD's long-specified-but-absent **`ConfigurationError`** and rewrote 46 inline parse sites, proven behaviour-preserving against a 110-field snapshot. **14 pass, 1 partial, 2 not run.** The remaining partial is T-8 (the restart half would write the untracked `.env` from a test). T-13/T-17 are the load pair below. | **Only T-8's deliberate exclusion remains** — a PO call | effectively done |
...
**TAC-3 — `FASTAPI_WORKERS` resolves to neither branch the plan anticipated, and then the launcher was fixed.** It is not inert, and it is not unconditionally in effect: `start.sh`'s non-`DEV_MODE` branch passed `--workers "${FASTAPI_WORKERS:-4}"`, so the key was honoured on that launcher and ignored on the four the docstring listed. That made it CONDITIONAL on which launcher ran, with two consequences worse than the mismatch the function was written to report — a configured value above 1 was a real second worker on that path rather than a contradiction sitting in `.env`; and `${...:-4}` substituted 4 when the key was unset **or empty**, so starting the stack that way without the key yielded four workers each holding their own copy of whisper and Kokoro, which is the crash the stack is designed to avoid, reached by doing nothing. The code's two false claims ("No launcher passes it to uvicorn", "No launcher honours the setting; the running count is 1") were corrected in place with the line reference.

**The launcher decision, taken 2026-09-21: the flag is removed, not tuned.** Phase 1.2 left `start.sh`'s default unchanged on the grounds that it was a launcher decision rather than a configuration migration. Taken as a launcher decision, the answer is that the flag should not exist at all: every other launcher starts one process, and `FASTAPI_WORKERS` is sizing metadata written by the hardware profiler, not a worker count. So `--workers "${FASTAPI_WORKERS:-4}"` is **gone** and `start.sh` starts a single process like `start_services.ps1`/`.sh`, the Dockerfile and docker-compose. Defaulting it to 1 was rejected because it fixes only the unset case — an explicit `FASTAPI_WORKERS=4` in `.env`, which this repo's `.env` carries at line 238, would still have produced four workers. The original claim is true again, but because no launcher passes the flag, rather than because the list of launchers was incomplete. `.env`'s `4` is deliberately left alone: it is a sizing figure, so it does not need to be 1. `app/main.py`'s report now says NOT IN EFFECT rather than CONDITIONAL, and `app/config.py`'s field comment — which also named `start.sh:164` — is corrected with it.

**Bucket D — closed 2026-09-21.** The engine-level `OLLAMA_*` keys now have a home and a carve-out:

- `.env.example` gains a section, "Consumed by the Ollama SERVICE, not by this application", naming the keys the `ollama serve` **process** reads — `OLLAMA_NUM_PARALLEL`, `OLLAMA_MAX_LOADED_MODELS`, `OLLAMA_FLASH_ATTENTION`, `OLLAMA_KV_CACHE_TYPE`, `OLLAMA_CONTEXT_LENGTH`, `OLLAMA_HOST`, `OLLAMA_MODELS`. The values in it are **measured, not remembered**: they are what this box's own `msg="server config"` startup line reports with nothing set. The section states plainly that these belong in the *service's* environment and that putting them in `.env` does nothing to the engine.
- **The first draft of that section contained a false claim, and checking it is what produced `OLLAMA_MODELS`.** It asserted that no launcher in the repository sets any engine key. `grep` says otherwise — `docker-compose.yml:55-56` sets `OLLAMA_HOST=0.0.0.0:11434` and `OLLAMA_MODELS` for its `ollama` container. That is this pattern done correctly rather than an exception to it (a container `environment:` block, not `.env`), so the text now names it as the in-repo example, and `OLLAMA_MODELS` — which the draft had simply missed — was added to both the section and the carve-out.
- `_EXTERNALLY_CONSUMED` gains those seven keys as an **exact list, deliberately not an `OLLAMA` prefix**. A prefix would silently excuse any future `OLLAMA_*` key that nothing reads — the one thing the sweep exists to catch — so the carve-out buys the false-positive fix without spending the gate's power. `OLLAMA_KEEP_ALIVE` is excluded on purpose: the engine reads that name and so does this application (`app/config.py`, `app/llm_backend.py`), so it is genuinely read and can never be inert. Its **two-consumer trap** is documented instead — the same name is our per-request `keep_alive` in `.env` and the server's request default in the service environment, and the measured 32,919 ms cold load came from conflating them.
- Two non-vacuity controls were added to `test_us011_config_truth.py` (38 → **40** checks), because a carve-out and a canary pull in opposite directions: a planted engine key must be reported externally-consumed, and a planted `OLLAMA_TUNED_BY_NOBODY` **must still be caught as inert**. The second is what keeps the exact-list decision honest; without it the carve-out could widen to a prefix and nothing would notice.

Two corrections to the plan's own wording came out of doing it. `SMALL_TASK_NUM_CTX` was already in `.env.example` (added commented by `5c5df80`, 2026-09-19) — and commented is right: the entry explains that leaving it unset lets every call site inherit `OLLAMA_NUM_CTX`, which is what avoids the runner thrash. `.env` pins it to 8192 deliberately, with its own reasoning in place. And the plan's note that `.env` should gain the VAD keys because "the harness reads them" is **not accurate**: `test_endpointing.py` *sets* `VAD_SILENCE_MS`/`VAD_SPECULATIVE_ADVANCE_MS` in-process and pops them again — that mutation is precisely why both keys are on the `DYNAMIC_KEYS` allowlist — and no harness reads either from `.env`. They were added to `.env` anyway, for the reason that does hold: they are resolved by `app/config.py` and `REC-05` makes `.env` the runtime authority, so the endpointing values an operator tunes should be visible where they are tuned. Note `.env` is **gitignored**, so that half is a local change and cannot be committed; only the `.env.example` half is.

Still to do in Phase 1.2, per the approved plan: `MACHINE_PROFILE_CHECK` implemented as a non-blocking boot drift report (discharges `DG-05` visibility); and a second carve-out corpus for `scripts/*.py` and the root scripts, which must never fold into the runtime corpus.

**Outstanding after Phase 1.2:** `MACHINE_PROFILE_CHECK` as a non-blocking boot drift report (`DG-05` visibility); a second carve-out corpus for `scripts/*.py` and the root scripts; and the LLD test mapping (T-1..T-17), `MOD-07` B.4/B.6, and the TAC-8 no-regression load test. Bucket **D** is closed — see above.

The one operational item this phase surfaced and deliberately did not fix — `start.sh`'s `${FASTAPI_WORKERS:-4}` default — **was fixed on 2026-09-21** as a launcher decision: the flag is removed rather than tuned, so every launcher starts a single process and the key is inert on all of them. See the TAC-3 note above.