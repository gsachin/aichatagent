...
- **Status:** **IMPLEMENTED - TAC-5 wired 2026-09-19; TAC-1 MIGRATION COMPLETE 2026-09-21 (Phase 1.2, zero unexplained reads; bucket D closed); AC-4/TAC-7 still partial; BRD-15 demonstrated** · `test_us011_config_truth.py` **51/51** · `test_us011_import_order.py` 13/13 · **DoD 6/8** · the two open boxes are sized in "Remaining DoD — assessed 2026-09-21": ~3–5 h, and only the TAC-8 load pair needs a quiet box · **TAC-6 implemented 2026-09-21** — provenance reported a constant until then, and the check asserting otherwise was enforcing the defect
...
the checks that actually exist: **thirteen pass, two are partial, two are not
run** (T-13/T-17, the TAC-8 load pair). The two partials are named with their
...
| T-15 | **PARTIAL — and the split is the finding** | A malformed value behaves differently by KEY CLASS, and only one class matches the scenario cleanly. A key parsed at import (`int(_env(...))`, e.g. `app/config.py:127`) **raises before the process can serve** — verified: `VAD_SILENCE_MS=notanumber` stops the import with `ValueError: invalid literal for int()`. That satisfies "stops the stack before it accepts a call", but the key is named only inside a traceback, not in a report. A key that survives import is caught by `check_config_keys`, which names it properly — but that makes the stack not-READY, it does not stop it. So: the stop is unattributed and the attribution does not stop. Both halves exist; no single path has both. |
...
- [ ] Tests from the LLD test scenarios pass (T-1 … T-17) — **mapped and driven 2026-09-21: 13 pass, 2 partial, 2 not run** (evidence table above). T-14, T-1, T-10, T-11 and T-12 are closed; closing T-1/T-10 is what exposed TAC-6 being unimplemented. The box stays open on **T-8** (the restart half would mean writing the untracked `.env` from a test — left partial deliberately) and **T-15** (the stop is unattributed and the attribution does not stop), plus the TAC-8 load pair
...
|---|---|---|---|---|
| 2 | LLD test mapping (T-1…T-17) | **Mapped and driven 2026-09-21**: T-14, T-1, T-10, T-11 and T-12 closed, and closing T-1/T-10 exposed **TAC-6 being unimplemented** — `source` was a constant, so 58 `Settings` fields had no reported provenance and no key could record the artifact as overridden; the report is now 122 rows with computed provenance, surfacing the three real `.env`-vs-artifact divergences. **13 pass, 2 partial, 2 not run.** The partials are T-8 (the restart half would write the untracked `.env` from a test) and T-15 (the stop is unattributed, the attribution does not stop). T-13/T-17 are the load pair below. | **Desk only** — T-15 wants a key-naming path for import-time parse failures | remaining ≈ 1–2 h |
