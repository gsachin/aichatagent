...
- [x] Module docs updated if contracts changed — `MOD-04` B.4 (`DAT-11` row gains the scope) and B.8 risk 3, whose recorded outcome is either "test passed, cache kept process-wide" or "test failed, per-call adopted" — never left as "may fail". **Verified as already satisfied 2026-09-19, and ticked on evidence rather than on the text being present:** B.4's `DAT-11` row carries the scope, the `(agent text, voice, speed)` key, the owning-call reference and its purpose ("not part of the key and never affects a lookup; it exists so a cross-call hit is measurable"); B.8 risk 3 records the outcome as **"it failed, and the pre-decided fallback was applied"** with the counts. Both were checked against as-built rather than accepted: the key really is `(tts_text, voice, speed)` (`voice_handler.py:1070`), the scope really is `per_call` in both `.env` and `.env.example`, and `_cache_key_sha` exists for attribution

**One nuance, recorded because `BRD-15` rests on it.** `TTS_CACHE_SCOPE` is read at **module import** (`voice_handler.py:188`), so an operator reverting in production edits `.env` and restarts. `test_brd15_rollback.py` instead assigns the module global directly (`vh.TTS_CACHE_SCOPE = "shared"`) — equivalent in effect, because the serving path consults that global at call time, but not identical in mechanism. The revert is genuinely **demonstrated** (the switch is flipped and the prior behaviour observed, then restored and re-checked), not merely asserted; it is worth knowing that the demonstration does not exercise the restart path
