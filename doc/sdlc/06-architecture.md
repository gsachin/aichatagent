...
|---|---|---|---|---|---|---|
| MOD-01 | stateful per-call session | concurrent call | concurrent calls > 2 | VRAM (~90% at N=2) | VRAM exhaustion, then carrier socket count | **BUILT 2026-09-19 (US-016): refuse, not queue.** The third call is refused at the carrier-facing endpoint — the busy TwiML plays a pre-synthesised asset and connects no media stream, so no session, no history and no KV allocation is created. A refused caller redialling into a freed slot is admitted as a fresh session. Recovery: the live session count is released when a session ends, so admission resumes with no operator action. **The queue alternative is rejected, not unconsidered:** a queue would hold a caller who has already been answered by the carrier, with no way to tell them why they are waiting, and BRD-13's failure-capability matrix requires a deterministic outcome rather than an unbounded wait. It reopens only if the concurrency ceiling rises meaningfully above 2, where refusing a third caller starts turning away real demand instead of protecting two live calls. |
...
**Degradation ladder:** (1) retrieval degrades to keyword-only; (2) retrieval degrades to local store; (3) TTS degrades to CPU; (4) generation queues; (5) new calls are refused with a spoken message. **Rung 5 is built (US-016):** the refusal is a pre-synthesised asset read from disk, so the rung itself costs no model and no synthesiser — which is what lets it hold when those are the things that failed. **US-017 adds the rung below it:** background work (chat/admin) is held off while any caller's turn is in flight and runs at most one at a time, so the class of work with no human waiting degrades before anything a caller can hear. `BRD-20`'s 2-voice + 1-background scenario is measurable by the harness as of 2026-09-19 (`--background-units`). Full service returns by the designed recovery path of each step. **"It fails" is not a mode — every rung above has a named recovery.**
