...
| `new_trace(call_id, turn_id)` | consumed from `MOD-01` at the end-of-speech decision | in-process function call | Opens the turn's trace and makes it *current*; returns it. **Was absent from this table** — `MOD-01` cannot emit anything without it | In-process |
| `mark_current(stage)` / `note_current(**fields)` / `emit_current()` | consumed from `MOD-01`–`MOD-04` | in-process function call | Mark the *current* trace resolved from a `ContextVar`. **Was absent from this table.** These exist because `MOD-02`'s retrieval mark is emitted from inside a `to_thread` worker: `asyncio.to_thread` copies the calling context, so the per-WebSocket `ContextVar` is visible there and two concurrent callers get distinct copies (`BRD-06`). Without these, retrieval time would be unattributable in exactly the concurrent case `MOD-02` is named for | In-process |
