...
| Single half-open probe on a timer | Matches the designed recovery in `06-architecture.md` §5; distinguishes "service is back" from "service is still down" without paying the full timeout to find out | Keep the flat 30 s cooldown — an outage costs the full read timeout on the first request after every cooldown window |
...
| MCP client | `app/rag_mcp.py` | **Refactor** | New `httpx.Client` per request → one reused client; flat breaker → probe-on-timer (TRD-09) |
