...
|---|---|---|---|---|
| `FASTAPI_WORKERS=4` | `.env:238`, `.machine_profile.json` `applied`, and every repo reference outside the test file is on the *writer* side (`app/hardware_profile.py:51`, whose two clamp sites are `:331` and `:341`) | The launcher starts uvicorn with **no `--workers` argument**, so the app runs as one process | `start_services.ps1:597`: `ArgumentList = "-m", "uvicorn", "app.main:app", "--host", "127.0.0.1", "--port", "$FastAPIPort"`. **This row became true again on 2026-09-21.** It was false as written: `start.sh`'s non-`DEV_MODE` branch passed `--workers "${FASTAPI_WORKERS:-4}"`, so the key WAS consumed on that launcher — the one launcher this row's evidence did not list. The flag has been removed (`US-011`, `TAC-3`), so "every repo reference is on the writer side" now holds without exception | **Remove** the key and its profile entry. The single-process design is deliberate (`06-architecture.md` §5 names the FastAPI process as a SPOF precisely because there is one), so wiring it would contradict a recorded architectural decision |
| Pipecat `VADParams(stop_secs=0.5, start_secs=0.3, confidence=0.7, min_volume=0.6)` | ~~`app/pipeline.py:174–180`~~ **removed 2026-09-19** | **Done — REMOVED, not marked.** The row above described the analyzer as "constructed and never referenced again"; it was, and the recommended remedy was to remove the block or mark it unmistakably non-live. It has been removed from `app/pipeline.py`, and the removal site carries the reason so it is not rebuilt. This was the single most dangerous inert value in the repository — it read as the endpointing configuration and sat 100 ms from the live 600 ms RMS gate that is 86% of the latency budget (`BRD-04`, `CV-02`). The live decision is now `VAD_SILENCE_MS` in `app/voice_handler.py`, reachable from configuration, and `BRD-04`'s "configuration that does not execute shall not be presented as the live setting" is satisfied on both counts |
...
| Serving-path `keep_alive` | `app/llm_backend.py:185` (`_chat_ollama`), `:204` (`ka = KEEP_ALIVE if keep_alive is None else keep_alive`) | **CLOSED 2026-09-19 — was Debt, not this module** | This row recorded the gap that neutralised this module's pre-warm: `_chat_ollama` sent only `num_ctx` and `temperature`, and Ollama resets a model's keep-alive to the server default on any request that omits it, so the boot pre-warm survived until the first chat and no longer. It is now sent on **every** generation, read from `OLLAMA_KEEP_ALIVE` (`.env:235`, default `-1`), with a loud degradation path if the installed client rejects the kwarg. `TRD-12` is satisfied; `MOD-07` still reports residency as a boot snapshot and still claims no more than that |
...
| `FASTAPI_WORKERS` key | `.env:238`, `.machine_profile.json`, `app/hardware_profile.py` | **Debt** | Remove under `TRD-25`; the single-process design is deliberate (`REC-02`) |
