...
| P10 | `pick_model()`'s uncached per-utterance `/api/tags` round trip | **still open**, *"remains a real per-turn cost"* (the `localhost` stall is fixed; the call itself is not) |
